...
This is a clone project of the popular word guessing game we all know and love. Made using React, Typescript, and Tailwind adapted for top-1000 password guessing

[**Try out the demo!**](https://passwle.vercel.app/)
...
```bash
$> docker build -t passwle:dev -f docker/Dockerfile .
$> docker run -d -p 3000:3000 --name passwle-dev passwle:dev
```
...
```bash
$> docker build --target=prod -t passwle:prod -f docker/Dockerfile .
$> docker run -d -p 80:8080  --name passwle-prod passwle:prod
```
